...
## Filter Query History by `history_id`

_June 25, 2025_

Vectara now supports filtering query history results by a specific history ID 
(trace ID) in the `/v2/queries` API endpoint. This enhancement enables users to 
retrieve an individual query history entry using the `history_id `query parameter.

**Why it matters:**

Tracing is essential for debugging and auditing conversational applications. 
With this new `history_id` filter, you can now isolate a single query history 
record. This makes it easier to track query execution, or build custom 
dashboards.
