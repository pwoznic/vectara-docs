...
business logic, data processing, or integrations.
...
The entry point must be `process`. This example calculates customer scores based on their activity metrics:
...
:::note Notes
- The function must be named `process`
- Use type hints for automatic schema discovery.
- Parameters with default values become optional in the schema.
- Return a **JSON-serializable value** (dict, list, string, number, boolean).
:::

**Step 2: Create the Lambda Tool with the API**
...
Lambda Tools support returning any JSON-serializable Python type:
...
<CodePanel
  ...
  snippets={[
    {
      language: "python",
      code: `# Return a dictionary (object)
def process(x: int, y: int) -> dict:
